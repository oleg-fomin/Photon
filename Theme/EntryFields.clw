...
!    
    !
		SELF.EntryQ.EntryimageFEQ = Create(0, CREATE:Box, ParentFeq)
		SELF.EntryQ.EntryimageFEQ{PROP:FillColor} = SELF.NormalColor
		SELF.EntryQ.EntryimageFEQ{PROP:Fill} = 002D2316h
		SELF.EntryQ.EntryimageFEQ{PROP:LineWidth} = 1
		SELF.EntryQ.EntryimageFEQ{PROP:Round} = TRUE
		SELF.EntryQ.EntryimageFEQ{PROP:Hide} = TRUE
...
savePixels                              BYTE
	...
	! Is it now hidden? Or something else? Shall we re-draw something? Resize Something?
		...
		IF SELF.EntryQ.PosChecksum <> pos.PosChecksum
			...
			SetPosition(SELF.EntryQ.EntryimageFEQ, pos.XPos - 5, pos.YPos - 4, pos.Width + 10, pos.Height + 8)
